...
define(
	'Cart.Summary.View', [
		'SC.Configuration', 'Cart.PromocodeForm.View', 'Backbone.CompositeView', 'Backbone.CollectionView', 'GlobalViews.FormatPaymentMethod.View', 'Profile.Model', 'Cart.Promocode.List.View', 'Session'

		, 'cart_summary.tpl', 'cart_summary_gift_certificate_cell.tpl'

		, 'underscore', 'Backbone', 'Utils'
	],
	function (
		Configuration, CartPromocodeFormView, BackboneCompositeView, BackboneCollectionView, GlobalViewsFormatPaymentMethodView, ProfileModel, CartPromocodeListView, Session

		, cart_summary_tpl, cart_summary_gift_certificate_cell_tpl
		...
		return Backbone.View.extend({
			...
			childViews: {
				...
				'GiftCertificates': function () {
					var gift_certificates = this.model.get('paymentmethods').where({
			...
			getContext: function getContext() {
				var continueURL = Configuration.get('siteSettings.sitetype') === 'ADVANCED' ? Configuration.defaultSearchUrl : '',
					...
					default_country = shipping_address.get('country') || SC.Tools.getDefaultCountry(),
					...
					,
					gift_certificates = this.model.get('paymentmethods').where({
						type: 'giftcertificate'
					}) || [],
					is_single_country = _.size(Configuration.get('siteSettings.countries', [])) === 1,
					is_zipcode_require, selected_country, countries_list, is_estimated_shipping_valid, show_estimation_form = true,
				...
				if (countries && default_country && countries[default_country]) {
					selected_country = _.findWhere(countries, {
						selected: true
					});
					if (selected_country) {
				...
				var isDefaultShipping = false; //Default hard coded shipping addresses pre site.
				var shipaddress = this.model.get("shipaddress");
				if (shipaddress == "AU---2000----null" || shipaddress == "GB---EC3N-4AB----null" || shipaddress == "US---94129----null" || shipaddress == "FR---75007----null") {
					isDefaultShipping = true;
				}
				var showTaxAndShipping = true;
				if (isDefaultShipping) {
					show_estimation_form = true;
				}
				//console.log(shipaddress+" "+showTaxAndShipping+" "+show_estimation_form+" "+isDefaultShipping);
				//console.log(this.model);

				var lines = this.model.get("lines");
				var site = SC.Tools.getSiteCode();
				var linemodel, subtotal = 0,
					incTax, extraTax;
				for (var i = 0; i < lines.length; i++) {
					linemodel = lines.models[i];
					//console.log(linemodel.get("total")+" "+parseFloat(linemodel.get("tax1_amount")));
					if (parseFloat(linemodel.get("tax1_amount")) > 0) {
						incTax = true;
						if (site != "UK" && site != "Europe") {
							extraTax = true;
						}
					}
					subtotal += linemodel.get("total") + parseFloat(linemodel.get("tax1_amount"));
				}
				...
				var discount = summary.discounttotal + summary.taxondiscount;
				var shipping = (summary.shippingcost || 0) + (summary.taxonshipping || 0);
				var handling = summary.handlingcost + summary.taxonhandling;
				// console.log("subtotal="+subtotal);
				// console.log("discount="+discount);
				// console.log("shipping="+shipping);
				// console.log("handling="+handling);
				var calctotal = subtotal - discount + shipping + handling;
if(discount>0){
				discount = -total + handling + shipping + subtotal;
}
				// console.log("discount="+total+" "+handling+" "+shipping+" "+subtotal);
				// console.log("discount="+discount);
				// console.log("calctotal="+calctotal);
				// console.log("total="+this.model.get("summary").total);
				//console.log(this.model);
				...
				return {
					...
					showTaxAndShipping: showTaxAndShipping,
					site: site,
					subtotal: subtotal,
					discount: discount,
					shipping: shipping,
					handling: handling,
					subtotal_formatted: Utils.formatCurrency(subtotal, symbol),
					discount_formatted: Utils.formatCurrency(discount, symbol),
					shipping_formatted: Utils.formatCurrency(shipping, symbol),
					handling_formatted: Utils.formatCurrency(handling, symbol),
					hasHandling: handling >= .01,
					hasDiscount: discount >= .01,
					hasSubtotal: subtotal >= .01,
					hasShipping: (subtotal >= .01) || (shipping >= .01),
					incTax: incTax,
					extraTax: extraTax,
					currency: SC.Tools.getCurrency()
