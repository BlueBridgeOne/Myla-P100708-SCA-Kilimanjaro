...
define(
	'OrderWizard.Module.Shipmethod', ['Wizard.Module', 'Profile.Model', 'GlobalViews.Message.View'
	...
	function (
		WizardModule, ProfileModel, GlobalViewsMessageView
			...
			getContext: function getContext() {
				...
				var self = this,
					...
					shipping_methods = this.model.get('shipmethods').map(function (shipmethod) {
						var price = "",
							char; //Turn the formatted price into a decimal.
						for (var i = 0; i < shipmethod.get('rate_formatted').length; i++) {
							char = shipmethod.get('rate_formatted').charAt(i);
							if (char == "0" || char == "1" || char == "2" || char == "3" || char == "4" || char == "5" || char == "6" || char == "7" || char == "8" || char == "9" || char == ".") {
								price += shipmethod.get('rate_formatted')[i];
							} else if (char == ",") {
								price += ".";
							}
						}

						var price_container_object = {
							price: parseFloat(price),
							price_formatted: shipmethod.get('rate_formatted')
						};
						var taxrate;
						if (hasTax) {
							taxrate = 20;
						}
						var message = SC.Tools.updatePriceObject(price_container_object, taxrate);
						return {
							name: shipmethod.get('name'),
							rate_formatted: price_container_object.price_formatted,
