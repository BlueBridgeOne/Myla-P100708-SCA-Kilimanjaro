...
define(
    ...
    function(
        BackboneView, Backbone, jQuery, _, Utils, PluginContainer
    ) {
        'use strict';
            ...
            _renderChildViewInstance: function _renderChildViewInstance(child_view_instance, placeholder, number_views) {
                    if (child_view_instance) {
                        ...
                        if (number_views === 1 && !child_view_instance.isCCT) {
                            ...
                            var $CMSImages = this.$(placeholder).find("img");
                            $CMSImages.each(function() {
                                var $self = $(this);
                                var src = $self.attr("data-src");
                                if (src&&$self.attr("src") != src) {
                                $self.css("opacity", .01);
                                $self.attr("data-src",undefined);
                                    console.log("Fade in image - " + $self.attr("data-src"));
                                    $self.attr("src", src).load(function() {
                                        $self.css("opacity", 1);
                                    });
                                }
                            });
