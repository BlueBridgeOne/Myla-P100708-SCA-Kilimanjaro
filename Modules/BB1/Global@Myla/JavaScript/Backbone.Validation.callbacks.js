	...
	,	invalid: function (view, attr, error, selector)
		{
			...
			var $target
			var	$control;
			//a bit of a hack to place errors in multiple locations.
			if(attr=="custcol_bb1_matrix_item_colour"||attr=="custcol_bb1_matrix_item_size"){
				$control = $('['+ selector +'="'+ attr +'"],['+ selector +'="header_'+ attr +'"],['+ selector +'="subheader_'+ attr +'"]');
				
			}else{
			$control = view.$el.find('['+ selector +'="'+ attr +'"]');
			}
			var	$group = $control.closest(selectors.build('controlGroup')).attr(selectors.error.attr, selectors.error.value);
