...
define(
	'ErrorManagement', [
		...
		, 'ErrorManagement.ExpiredLink.View', 'ErrorManagement.ForbiddenError.View', 'ErrorManagement.InternalError.View', 'ErrorManagement.LoggedOut.View', 'ErrorManagement.PageNotFound.View', 'ErrorManagement.ResponseErrorParser', 'GlobalViews.Message.View'
	...
	function (
		...
		, ExpiredLinkView, ForbiddenErrorView, InternalErrorView, LoggedOutView, PageNotFoundView, ErrorManagementResponseErrorParser, GlobalViewsMessageView

		, jQuery, Backbone, _, ProfileModel, Utils
	) {
		'use strict';
			...
			showError: function (message, type, closable, disableElements) {
					...
					if (message.indexOf(payment) > -1) {
						...
						if (this.options&&this.options.application) {
							...
							SC.Tools.showErrorInModal(this.options.application, "Payment", message);
						}
					}
				}
		...
		return {
			...
			mountToApp: function (application) {
				...
				jQuery(document).ajaxError(function (e, jqXhr, options, error_text) {
					...
					console.log("AJAX ERROR "+error_text+" "+intStatus);
