        ...
        return {
            ...
            _thumbnail: function (item) {
                    ...
                    if (images.length) {
                        ...
                        if (colour) { //Attempt to match the colour to the url if possible.
                            colour = colour.toLowerCase();
                            for (var i = 0; i < images.length; i++) {

                                if (images[i].url.indexOf("-" + colour + ".") > -1 || images[i].url.indexOf("_" + colour + ".") > -1 || images[i].url.indexOf("_" + colour + "-") > -1) {
                                    return images[i];
                                }
                            }
                        }
            ...
            _priceDetails: function (item) {
                    // var priceLevel=SC.SESSION.priceLevel;
                    if (SC.Tools.isUSPrice()) {
                        var p = item.get('pricelevel' + SC.Tools.USPriceLevel);
                        var q = item.get('pricelevel' + SC.Tools.USPriceLevel + "_formatted");
                        return {
                            onlinecustomerprice: p,
                            onlinecustomerprice_formatted: q
                        };

                    }
                    // else if(priceLevel=="5"){
                    //     var p = item.get('pricelevel5');
                    //     var q = item.get('pricelevel5_formatted');
                    //         return {onlinecustomerprice:p,onlinecustomerprice_formatted:q};
            ...
            _price_formatted: function (item) {
                    //var priceLevel=SC.SESSION.priceLevel;
                    if (SC.Tools.isUSPrice()) {
                        var p = item.get('pricelevel' + SC.Tools.USPriceLevel + "_formatted");
                        if (p) {
                            return p;
                        }
                    }
                    // else if(priceLevel=="5"){
                    //     var p = item.get('pricelevel5_formatted');
                    //     if (p) {
                    //         return p;
                    //     }
                    // }
            ...
            _comparePriceAgainst: function (item) {
                    ...
                    if (prices) {
                        ...
                        if (prices.priceschedule) {
                            ...
                            //calculate compare using RRP fields.
                            var rrp = SC.Tools.getRRP(item);
                            //rrp *= 1.1; testing
                            if (rrp && rrp > prices.onlinecustomerprice) {
                                return rrp;
                            }
                    ...
                    if (prices) {
                        if (prices.priceschedule) {
                            ...
                            if (rrp && rrp > prices.onlinecustomerprice) {
                                ...
                                return Utils.formatCurrency(rrp, symbol);
                            }
