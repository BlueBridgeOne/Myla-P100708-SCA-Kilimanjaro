...
define(
	'Application', [
		'underscore', 'Events', 'SC.ComponentContainer', 'SC.Models.Init', 'Configuration', 'Application.Error', 'Utils', 'Console'
	],
	function (
		_, Events, SCComponentContainer, ModelsInit, Configuration, ApplicationError, Utils
	) {
		'use strict';
		...
		var Application = _.extend({
			...
			getEnvironment: function getEnvironment(request) {
					...
					if (result.currentCurrency) {
						...
						if (this.isSecure && ModelsInit.session.isLoggedIn2()) {
							...
							// //check if currency if fixed by seeing if it can be changed.
							// var swapCurrency = 1;
							// var testCurrency = ModelsInit.session.getShopperCurrency();
							// //nlapiLogExecution("DEBUG","testCurrency="+JSON.stringify(testCurrency));
							// if (swapCurrency == testCurrency.internalid) {
							// 	swapCurrency = 2;
							// }

							// //nlapiLogExecution("DEBUG","swapCurrency="+JSON.stringify(swapCurrency));
							// //try swapping
							// ModelsInit.session.setShopperCurrency(swapCurrency);
							// var newTestCurrency = ModelsInit.session.getShopperCurrency();
							// //nlapiLogExecution("DEBUG","new testCurrency="+JSON.stringify(newTestCurrency));
							// if (testCurrency.internalid == newTestCurrency.internalid) {
							// 	//The swap failed, so currency is fixed.
							// 	result.fixedCurrency = true;
							// } else {
							// 	//swap back
							// 	ModelsInit.session.setShopperCurrency(result.currentCurrency.internalid);
							// 	result.fixedCurrency = false;
							// }
						}
					}

					try {
						var list = ModelsInit.session.getCustomer().getCustomFieldValues();
						for (var i = 0; i < list.length; i++) {
							if (list[i].name == "custentity_bb1_sca_iscustomer") {
								result.fixedCurrency = list[i].value == "T";
								break;
							}
						}
						
						//nlapiLogExecution("DEBUG","Get Fields","fixed="+fixed);
						//nlapiLogExecution("DEBUG","Std",JSON,stringify(ModelsInit.customer.getFieldValues()));
					} catch (err) {
						//nlapiLogExecution("DEBUG", "Get Fields", err.toString());
					}
			...
			getPermissions: function getPermissions() {
					...
					var purchases_permissions = [
							ModelsInit.context.getPermission('TRAN_SALESORD'), ModelsInit.context.getPermission('TRAN_CUSTINVC'), ModelsInit.context.getPermission('TRAN_CASHSALE')
						],
						purchases_returns_permissions = [
							ModelsInit.context.getPermission('TRAN_RTNAUTH'), ModelsInit.context.getPermission('TRAN_CUSTCRED')
						];
