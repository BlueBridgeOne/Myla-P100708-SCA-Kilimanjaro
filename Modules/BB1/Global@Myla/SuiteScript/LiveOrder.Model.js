...
define(
	'LiveOrder.Model', [
		'SC.Model', 'Application', 'Profile.Model', 'StoreItem.Model', 'SC.Models.Init', 'SiteSettings.Model', 'Utils', 'ExternalPayment.Model', 'underscore', 'CustomFields.Utils'
	],
	function (
		SCModel, Application, Profile, StoreItem, ModelsInit, SiteSettings, Utils, ExternalPayment, _, CustomFieldsUtils
	) {
		'use strict';
		...
		return SCModel.extend({
			...
			setSite: function () {
				...
				var bb1_sca_site = context.getSessionObject("bb1_sca_site");
				var site = request.getParameter("site");
				//nlapiLogExecution("DEBUG", "site", "Site is " + bb1_sca_site + " new site is " + site);
				if (site && site.length > 0 && bb1_sca_site != site) {
					//nlapiLogExecution("DEBUG", "site", "Set site to " + site);

					context.setSessionObject("bb1_sca_site", site);
					if (bb1_sca_site) {
			...
			submit: function submit(threedsecure) {
					...
					if (!threedsecure && payment && payment.paymentterms === 'CreditCard' && SC.Configuration.isThreeDSecureEnabled) {
						...
						nlapiLogExecution('DEBUG', 'Order', 'Submit');
						confirmation = ModelsInit.order.submit();
						nlapiLogExecution('DEBUG', 'Order', 'confirmation=' + JSON.stringify(confirmation));
					...
					nlapiLogExecution('DEBUG', 'Order', 'final confirmation=' + JSON.stringify(confirmation));
					return confirmation;
				}
			...
			setShippingAddress: function setShippingAddress(data, current_order) {
					if (data.shipaddress !== current_order.shipaddress) {
						if (data.shipaddress) {
							...
							nlapiLogExecution("DEBUG", "data.shipaddress", JSON.stringify(data.shipaddress));
			...
			forceVAT: function forceVAT(siteChanged) {
				var order_fields = ModelsInit.order.getFieldValues(['shipaddress'], false);
				//nlapiLogExecution('ERROR', 'order_fields', 'order_fields=' + JSON.stringify(order_fields));
				var shipaddress = order_fields.shipaddress


				var bb1_sca_site = context.getSessionObject("bb1_sca_site");


				//uk EC3N 4AB - tower of london
				//us 94129 - Golden gate bridge
				//europe 75007 - Effel Tower
				//rest of the world 2000 - Sydney Opera House



				//nlapiLogExecution('ERROR', 'Estimate', 'Update ' + bb1_sca_site + ' shipping for current ' + JSON.stringify(shipaddress));
				var Country = shipaddress.country,
					Zip = shipaddress.zip;


				if (!Country || !Zip || siteChanged) {
					switch (bb1_sca_site) {
						case "UK":
							if (Country == "GB"&&Zip=="EC3N 4AB") {
								return;
							}
							Country = "GB";
							Zip = "EC3N 4AB";
					...
					if (Country) {
						...
						var address_internalid = Zip + '-' + Country + '-null';
						var addr = {
						...
						ModelsInit.order.estimateShippingCost(addr);

						//nlapiLogExecution('ERROR', 'Estimate', 'SET ' + bb1_sca_site + ' new ' + JSON.stringify(addr));
					}
				}
			}
		});
