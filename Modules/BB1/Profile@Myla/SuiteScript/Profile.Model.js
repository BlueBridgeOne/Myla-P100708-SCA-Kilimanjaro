	...
	,	get: function ()
		{
			...
			if (ModelsInit.session.isLoggedIn2() && this.isSecure)
			{
				...
				var customerCustomFields = customer.getCustomFieldValues();
				console.log(JSON.stringify(customerCustomFields));
				var canpreview;

				for (var i=0; i < customerCustomFields.length; i++) {
					if (customerCustomFields[i].name == 'custentity_bb1_mya_allow_preview') {
						canpreview = customerCustomFields[i].value;
					}
				}

				profile.canpreview = canpreview;
			}
			else
			{
