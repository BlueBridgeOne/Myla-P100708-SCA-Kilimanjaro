	...
	,	initialize: function ()
		{
			...
			this.facet_config = this.options.translator.getFacetConfig(this.model.get('id'));
...
start=Utils.formatCurrency(start,symbol);
end=Utils.formatCurrency(end,symbol);
			this.$el
	...
	,	updateRangeSelection: function (e, slider)
		{
			...
			else if (!facet_values || parseFloat(facet_values.from) !== slider_values.low || parseFloat(facet_values.to) !== slider_values.high)
			{
				...
				var url=translator.cloneForFacetId(this.facetId, {
					from: slider_values.low.toFixed(2)
				,	to: slider_values.high.toFixed(2)
				}).getUrl();
				console.log(url);
// 				var wrong="price=%5Bobject%2520Object%5D";
// var pricelevel=5;
// 				if (SC.Tools.isUSPrice()) {
// 					pricelevel=7;
// 				}
// 				var right="pricelevel"+pricelevel+".from="+slider_values.low.toFixed(2)+"&pricelevel"+pricelevel+".to="+slider_values.high.toFixed(2);
				...
				_.each(original_values, function(value)
				{
					if (value.url !== '')
					{
						...
						if (facet_config.colors)
						{
							...
							}else if(value.color&&value.color.indexOf(';')>-1){
								
								var p=value.color.split(';');
								if(p.length==2){
									value.is2ColorTile = true;
								value.color2 = p[1];
								value.color = p[0];
								}else{
