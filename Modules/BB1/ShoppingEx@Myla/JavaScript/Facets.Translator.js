...
define('Facets.Translator', ['underscore', 'jQuery', 'SC.Configuration', 'Utils', 'UrlHelper', 'Tools'], function(
    _, jQuery, Configuration, Tools
) {
    'use strict';
    ...
    _.extend(FacetsTranslator.prototype, {
        ...
        parseUrl: function parseUrl(url) {
                ...
                if (this.isCategoryPage) {
                    facets = facets.replace(this.getCategoryUrl, '');
        ...
        getFacetConfig: function getFacetConfig(name) {
                ...
                var result = _.find(this.configuration.facets, function(facet) {
                    //console.log("- "+facet.id+" "+name);
                    return facet.id === name;
                    switch (name) {
                        case "price":
                            return facet.id === name || facet.id == "pricelevel5";
                        case "size":
                            return facet.id === name || facet.id == "custitem_matrix_bb1_item_size";
                        case "colour":
                            return facet.id === name || facet.id == "custitem_bb1_matrix_item_colour";
                    }
                    return facet.id === name;
                });
                ...
                var newFacet= result || _.extend({ id: name, name: name, url: name, isParameter: undefined }, this.defaultFacetConfig);
                if(newFacet.id=="price"){
                    newFacet.behavior="range";
                }
                return newFacet;
            }
...
if(this.categoryUrl){
        ...
        getApiParams: function getApiParams() {
                    ...
                    switch (facet.config.behavior) {
                        case 'range':
                            ...
                            if(facet.id=="price"){
                                ...
                                params['onlinecustomerprice.from'] = value.from;
                                params['onlinecustomerprice.to'] = value.to;
                            }else{
                ...
                if (this.isCategoryPage) {
                    ...
                    if (Q > -1) {
                        params.offset = this.options.show * (parseInt(params.commercecategoryurl.substring(Q + 5)) - 1);
                    }
        ...
        cloneForOption: function cloneForOption(option_id, option_value) {
                ...
                return new FacetsTranslator(facets, options, this.configuration, this.getCategoryUrl());
            }
